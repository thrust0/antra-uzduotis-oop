...
void input_method(list<Students>& group, int& menu_option)
{
    while(true)
    {
        ...
        else if(menu_option == 4)
        {
            ...
            auto it = FILE_PATHS.begin();
            advance(it, file_option - 1);
            file_input(group, *it);
            break;
        }
        else if(menu_option == 5) //generavimas studentu faile
        {
            ...
            auto it = STUDENT_COUNTS.begin();
            advance(it, amount_option -1);
            int student_amount = *it;
